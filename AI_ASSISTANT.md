...
Before any GPU/NPU/accelerator work specifically, also read SKB's `devices/redmi-turbo-4-pro/accelerator-access-audit.md` — the canonical, cross-project record of what this device's Vulkan/OpenCL/QNN-Hexagon doors actually allow. As of 2026-09-03 end-of-day: `ggml-hexagon` requalification is **in progress, not concluded either way** — a chain of real build/packaging bugs (namespace bridge, `ADSP_LIBRARY_PATH`, and an AGP CMake target-list bug that silently dropped the HTP skel `.so` builds entirely) has been found and fixed; CI run `33790288969` tests the last of these fixes and device requalification is the deferred next step. Read `docs/device-results/2026-09-03-redmi-turbo-4-pro-hexagon-htp-skels-not-built-root-cause.md` first (most recent, has the exact next action), then `PROJECT_STATE.md` §1.1/§4.2 for the full chain. Separately, Local Dream (a real, unprivileged third-party app) already proved genuine QNN-based NPU access is possible on this device in principle — `docs/device-results/2026-09-03-redmi-turbo-4-pro-hexagon-real-npu-path-found.md` — a fallback candidate if `ggml-hexagon` still doesn't pan out. `docs/HANDOFF-2026-09-03-npu-android-ecosystem-research.md` and `docs/device-results/2026-09-03-redmi-turbo-4-pro-hexagon-v73.md` carry earlier, partially-superseded findings — each has its own correction notice at the top.
