...
3. **Adreno OpenCL track — closed as a shipping concern, still open as a localization
   ...
   OpenCL's own registration is the next untested candidate, not a confirmed cause. `llama-opencl`
   remains not viable for shipping regardless (gated behind `BuildConfig.VALIDATED_ACCELERATORS`,
   empty by default) — that part is settled — but do not treat "OpenCL is the blocker" as fact
   until it has its own instrumented evidence. Full chain: `docs/device-results/2026-09-04-redmi-turbo-4-pro-chatterui-opencl-reference-investigation.md`,
   ...
   `docs/device-results/2026-09-04-redmi-turbo-4-pro-vulkan-instance-init-diagnostic.md`. Separately,
