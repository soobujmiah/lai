...
| Hexagon NPU (HTP) | **`ggml-hexagon` compiles clean (2026-09-03)**; no `llama-hexagon` backend selectable yet | SM8735's HTP confirmed **v73** via on-device evidence. CI now extracts the real Hexagon SDK 6.6.0.0 from the public `ghcr.io/snapdragon-toolchain/arm64-android:v0.7` image (opt-in only via `validated_accelerators=llama-hexagon`, never on ordinary builds) and wires `GGML_HEXAGON`/`HEXAGON_SDK_ROOT`/`HEXAGON_TOOLS_ROOT`/`PREBUILT_LIB_DIR` into the existing `runtime:llama` CMake build, same shape as OpenCL/Vulkan. Two real build failures diagnosed and fixed from actual CI logs (a `docker cp` permission bug on the SDK's read-only directories; a missing `PREBUILT_LIB_DIR` preset variable causing a CMake `string(FIND)` argument-count error) — full release APK now builds successfully with all four HTP skels (v73/v75/v79/v81) compiled in. Full record: `docs/HANDOFF-2026-09-03-npu-hexagon-scoping.md` | Stage 2 in progress: register `llama-hexagon` end to end (native backend adapter, `backend_registry.cpp`, Kotlin `InferenceScheduler` evidence gate, `core/model` catalog entry) before any real device qualification is possible |
