...
Snapshot date: 2026-09-03 (continued): the HTP-skel-targets fix (`4d90f37`) turned out to need two
follow-on fixes before it actually produced a usable build — an unconditional CMake target list
that broke non-Hexagon builds (`5c74806`), then a CMake-version floor (`ggml-hexagon/htp` needs
>=3.22.2, LAI had 3.22.1) fixed by pinning CMake 3.31.6 in CI (`a8954cb`, path-derivation
follow-up `21791ac`) — and finally an APK-packaging gap (the four HTP skels built but were never
copied into `lai_runtime`'s native-lib output, so they never reached the APK), fixed via a
CMake `POST_BUILD` copy in `runtime/llama/src/main/cpp/CMakeLists.txt` (`9ef2edd`). **CI run
`33812084740` is green and the release APK's `lib/arm64-v8a/` has been independently verified
(downloaded and unzipped, not just read from the build log) to contain all four
`libggml-htp-v7{3,5,9,1}.so` files, correctly sized, with no unrelated `.so` swept in.** This is
build-verified and packaging-verified only — **no device qualification has been run against this
APK yet; that is the very next step, deferred to next session.**
...
Head: **`9ef2edd`** · CI run **`33812084740` green** (release APK packaging fix, verified) · prior
Hexagon-track commits this session: `5c74806` (target-list gating), `a8954cb`+`21791ac` (CMake
3.31.6 pin), `9ef2edd` (HTP skel packaging) · **~192 `@Test` annotations** (187 at the 2026-08-19
snapshot; counted by grep, not re-verified against the CI-reported figure) · 16 Gradle modules

> **Session-close handoff (2026-09-03, continued):** the next session's first action should be
> installing the `lai-release-<run>` artifact from CI run `33812084740` (or a fresh dispatch with
> `validated_accelerators=llama-hexagon` if that artifact has expired) on the Redmi Turbo 4 Pro
> and running `scripts/device/lai_adb.sh qualify <model-id> llama-hexagon` for the real
> pass/fail/error-code result — this is the first time a build with all four HTP skels actually
> packaged has existed, so this is genuinely the next actionable step, not a re-investigation.
> Delete the pulled artifact after use per the artifact-hygiene rule
...
2. **Hexagon NPU (`llama-hexagon`) — actively being requalified, not a dead end and not yet
   proven working either; pending CI + device confirmation, deferred to next session.** Chain of
   fixes this session (`libcdsprpc.so` namespace bridge → `ADSP_LIBRARY_PATH` + legacy packaging
   → AGP CMake `targets` list, see §1.1 for the full trace and doc links) closed every gap found
   so far. **Immediate next step:** check CI run `33790288969`, confirm HTP skels are now in the
   built APK, install, and run `scripts/device/lai_adb.sh qualify llama-hexagon` on the Redmi
   Turbo 4 Pro. Do not re-derive the investigation — read
   `docs/device-results/2026-09-03-redmi-turbo-4-pro-hexagon-htp-skels-not-built-root-cause.md`
   first, it has the exact log evidence and fix. Separately, Local Dream already proved real QNN-
   based NPU access is possible on this device in principle
   (`docs/device-results/2026-09-03-redmi-turbo-4-pro-hexagon-real-npu-path-found.md`) — if the
   `ggml-hexagon` requalification above still fails after this fix, a QNN SDK integration
   (bundling `libQnnHtp*.so`, matching Local Dream's approach) remains the fallback candidate,
   not `ggml-hexagon`'s direct-FastRPC code. MLC Chat re-tested the same day showed no NPU use
   (OpenCL/CPU only, 4.2 tok/s) — unaffected by any of this.
3. **Adreno OpenCL track retry** — unrelated to the Hexagon finding above (that was a genuine
   ...
   and still needs its own fix: a background-thread + timeout-guarded probe around
   `initialize_llama_once()`, since that hang occurs during app launch itself, before the
   `<uses-native-library>` manifest change (currently reverted, not applied) can be safely
   reapplied.
