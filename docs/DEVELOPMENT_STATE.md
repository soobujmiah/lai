...
| **GPU Vulkan** | **Adapter implemented; device qualification in progress — Adreno driver issue** | Real `VulkanBackend::open()` with full layer offload via `llama_session` (build-verified CI #158+); device probe accepts `GGML_BACKEND_DEVICE_TYPE_IGPU` (Adreno 825 is integrated) — **device evidence 2026-08-19 (0.1.166):** Vulkan selected + model loads (2659 ms), but generation fails `vk::Device::createComputePipeline: ErrorUnknown` (Adreno driver rejects a compute pipeline; coopmat disabled via `GGML_VK_DISABLE_COOPMAT[_2]` workaround). Auto-fallback: accelerator generation failure/stall reloads the model on CPU so the app stays usable; no acceleration is claimed until device-validated |
