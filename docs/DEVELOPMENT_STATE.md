...
| **GPU Vulkan** | **Adapter implemented; shader-level driver workaround in place; device qualification in progress** | Real `VulkanBackend::open()` + full layer offload (`LLAMA_LOAD_MODE_NONE` for GPU; IGPU device accepted). **Device evidence 2026-08-19:** GPU context + KV cache on Vulkan0, generation fails compiling `mul_mat_vec_q4_k_f32_f32` (Adreno 825 driver bug). Fix: `GGML_VK_DISABLE_MMVQ` (+ coopmat disables) so the standard `mul_mat` shaders are used; `std::cerr` redirected to `LAI-llama` logcat so any remaining pipeline failure names the shader. Auto-fallback to CPU on accelerator failure/stall remains. |
