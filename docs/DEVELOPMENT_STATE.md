...
| **GPU Vulkan** | **Unsupported on this device — Qualcomm driver bug (addr2line-verified); CPU is the default** | Real `VulkanBackend::open()` + full layer offload (`LLAMA_LOAD_MODE_NONE`; IGPU probe). MMVQ compile failure fixed via LAI patch `ggml-vulkan-skip-mmvq.patch` (verified CI #175), but the **Adreno 825 driver then crashes natively during Vulkan compute** (release 0.1.175: process restart on message send — not catchable in Kotlin). Decision: **CPU is the default; GPU is opt-in** (`validated_accelerators=llama-vulkan`) until a qualified device/driver. Coopmat env disables + `std::cerr`→`LAI-llama` + in-app failing-shader capture + auto CPU fallback remain for future qualification. |
| **GPU OpenCL (Adreno track)** | **Closed on this device — device-policy wall; backend dormant** | llama.cpp's Qualcomm-maintained OpenCL backend (Adreno-optimized kernels) compiled into `liblai_runtime.so`; Khronos headers + ICD loader fetched by immutable SHA on CI only and linked statically (source-only policy). Backend `llama-opencl`, catalog rev 5 fallback, `DEVICE_VALIDATED` gate via `validated_accelerators=llama-opencl`, device pinning `GPUOpenCL`+CPU. 2026-08-20 device evidence: OpenCL stack healthy (OpenCL-Z: OpenCL 3.0 Adreno 825) but HyperOS exposes `libOpenCL.so` to no modern-app namespace (linker trace + empty `ld.config.txt` grep); dormant backend self-activates if a future build publishes it. Record: `docs/device-results/2026-08-20-redmi-turbo-4-pro-opencl-device-facts.md`. |
