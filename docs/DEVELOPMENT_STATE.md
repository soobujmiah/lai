...
| **GPU OpenCL (Adreno track)** | **Build verified — device qualification pending (primary GPU path)** | llama.cpp's Qualcomm-maintained OpenCL backend (Adreno-optimized kernels) compiled into `liblai_runtime.so`; Khronos headers + ICD loader fetched by immutable SHA on CI only and linked statically (source-only policy). Backend `llama-opencl`, catalog rev 5 fallback, `DEVICE_VALIDATED` gate via `validated_accelerators=llama-opencl`, device pinning `GPUOpenCL`+CPU. Next: qualification build on the Redmi Turbo 4 Pro (docs/BUILD_AND_RELEASE.md § Adreno OpenCL track). |
