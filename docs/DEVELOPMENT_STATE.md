...
| **GPU Vulkan** | **Adapter implemented; MMVQ-compile fix in place (release-175+); device qualification in progress** | Real `VulkanBackend::open()` + full layer offload (`LLAMA_LOAD_MODE_NONE` for GPU; IGPU device accepted). **Device evidence 2026-08-19 (0.1.173):** GPU context + KV cache on Vulkan0, generation fails compiling `mul_mat_vec_q4_k_f32_f32` (Adreno 825 driver bug). Root cause: ggml-vulkan PRE-COMPILES the whole dequant mul_mat_vec (MMVQ) shader family at device init regardless of `GGML_VK_DISABLE_MMVQ` (that env only stops them being *used*). Fix: LAI patch `scripts/ci/ggml-vulkan-skip-mmvq.patch` (applied by `fetch_llama_cpp.sh`) skips compiling the MMVQ family when disabled — the standard `mul_mat` shaders handle the work; verified applied in CI (#175). Coopmat env disables + `std::cerr`→`LAI-llama` + in-app failing-shader capture + auto CPU fallback remain. **Retest release-175+; then record `docs/device-results/` evidence.** |
