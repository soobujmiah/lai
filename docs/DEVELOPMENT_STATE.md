...
| **GPU Vulkan** | **Adapter implemented; offload root-caused; device qualification in progress** | Real `VulkanBackend::open()` with full layer offload via `llama_session` (build-verified CI #158+); device probe accepts `GGML_BACKEND_DEVICE_TYPE_IGPU`. **Device evidence 2026-08-19:** Vulkan selected + model loads, but `offloaded 0/29 layers to GPU` — because the forced `LLAMA_LOAD_MODE_MMAP` made llama.cpp swap the IGPU's preferred `Vulkan_Host` buffer to CPU (loader: "avoid using a host buffer when using mmap"), so weights never left CPU, and generation then hit `createComputePipeline: ErrorUnknown`. Fix: `LLAMA_LOAD_MODE_NONE` for GPU offload (weights land in Vulkan buffers) + `std::cerr` redirect to logcat so a driver pipeline failure names the exact shader. Auto-fallback to CPU on accelerator failure/stall remains. |
