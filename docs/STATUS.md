...
| **Adreno OpenCL track (GPU qualification primary path)** | **Closed on this device (device-policy wall) — backend dormant** | llama.cpp's Qualcomm-maintained OpenCL backend compiled into `liblai_runtime.so` (`GGML_OPENCL`, Adreno-optimized kernels embedded; Khronos OpenCL-Headers + ICD loader fetched by immutable SHA on CI only, ICD loader linked statically — no binaries committed). Backend `llama-opencl` in catalog **rev 5** (`llama-cpu` preferred; OpenCL/Vulkan fallbacks), same `DEVICE_VALIDATED` gate (`validated_accelerators=llama-opencl`), `model_params.devices` pinned to `GPUOpenCL`+CPU so dual-GPU builds offload deterministically, flash attention off for GPU, Kotlin auto CPU fallback covers OpenCL. **2026-08-20 device evidence:** the Adreno OpenCL stack is healthy (OpenCL-Z: OpenCL 3.0, Adreno 825, FULL_PROFILE) but HyperOS publishes `libOpenCL.so` to NO app namespace (`/linkerconfig/ld.config.txt` grep empty; linker trace refuses the dlopen) — only legacy-targeting apps bypass that config. Backend stays dormant and self-activates if Xiaomi publishes the library; record in `docs/device-results/2026-08-20-redmi-turbo-4-pro-opencl-device-facts.md`. |
