...
| User-owned `/sdcard/LAI/` workspace | Build verified (adapter pending device test) | `platform:workspace` owns the SAF tree grant (`ACTION_OPEN_DOCUMENT_TREE` + persistable permission, no `MANAGE_EXTERNAL_STORAGE`, no raw paths), canonical child resolution, a bounded discovery traversal (depth/count/size caps, streamed SHA-256, reviewed-catalog match via the pure classifier) and a temp-write-then-replace settings store backed by the pure codec; pure decisions and the GGUF magic detector are unit-tested; Chat ⚙ UI slice and physical SAF grant/discovery remain pending |
