...
| User-owned `/sdcard/LAI/` workspace | Build verified (core decisions) | typed workspace contracts, a pure discovery classifier (digest dedup, size/format/count limits, reviewed-catalog match) and a bounded settings codec (strict bytes/JSON/schema, fallback-to-defaults read, exact-schema write) are pure core and unit-tested; the Android SAF adapter (tree grant, child resolution, traversal) and UI remain pending |
