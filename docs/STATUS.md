...
| **Vulkan GPU adapter** | **Opt-in; CPU is the default** | Real `VulkanBackend::open()` with full layer offload (`LLAMA_LOAD_MODE_NONE`, IGPU probe); MMVQ compile failure fixed via LAI patch (`ggml-vulkan-skip-mmvq.patch`, CI #175) but the **Adreno 825 driver crashes natively during Vulkan compute** (0.1.175, process restart on send — uncatchable in Kotlin). Per the no-claim-without-evidence rule, `validated_accelerators` default is **empty = CPU-only**; `llama-vulkan` is opt-in for qualified devices. Coopmat disables, `std::cerr`→`LAI-llama`, in-app shader capture, and Kotlin-level CPU fallback remain. |
