...
| **Adreno OpenCL track (GPU qualification primary path)** | **Build verified — device qualification pending** | llama.cpp's Qualcomm-maintained OpenCL backend compiled into `liblai_runtime.so` (`GGML_OPENCL`, Adreno-optimized kernels embedded; Khronos OpenCL-Headers + ICD loader fetched by immutable SHA on CI only, ICD loader linked statically — no binaries committed). Backend `llama-opencl` in catalog **rev 5** (`llama-cpu` preferred; OpenCL/Vulkan fallbacks), same `DEVICE_VALIDATED` gate (`validated_accelerators=llama-opencl`), `model_params.devices` pinned to `GPUOpenCL`+CPU so dual-GPU builds offload deterministically, flash attention off for GPU, Kotlin auto CPU fallback covers OpenCL. Qualification build: workflow_dispatch `validated_accelerators=llama-opencl`. |
