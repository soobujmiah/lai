...
| **Vulkan GPU adapter** | **Implemented; device qualification pending** | Real `VulkanBackend::open()` with full layer offload (`LLAMA_LOAD_MODE_NONE` for GPU, mmap kept for CPU), IGPU probe (Adreno 825 is integrated), `GGML_VK_DISABLE_COOPMAT/_2` + `GGML_VK_DISABLE_MMVQ` for the confirmed failing shader `mul_mat_vec_q4_k_f32_f32`, `std::cerr`→`LAI-llama` + in-app capture of failing pipeline name, auto CPU fallback on accelerator failure/stall (fail-closed). Device evidence shows GPU context + KV cache on `Vulkan0`; generation blocked only by the driver shader bug. **Retest release-172+; then record `docs/device-results/` evidence.** |
