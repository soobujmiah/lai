...
The connected UI can install this entry with one tap. The air-gapped UI opens Android's file picker and accepts only bytes matching the same reviewed SHA-256, exact size and GGUF signature. Manual URL entry remains Developer Mode in the connected edition only.
