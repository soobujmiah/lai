...
*   **Storage:** `ModelRepository` (`noBackupFilesDir/models`, `registry.json` atomic `tmp→rename`, `SHA-256` streaming `8192`, `GGUF` magic `ModelFormatDetector`, `4 GiB` file cap, `64` files cap) + `WorkspaceDiscovery` (`storage/LAI/models` SAF `depth 4`, `256` cap, `8 GB` hard cap, `SHA` streaming, `WorkspacePolicy` `REVIEWED/LOCAL_UNREVIEWED/REJECTED`) + **auto-import** `storage/LAI/models/*.gguf` — runs at launch when the grant is active, whenever the workspace grant becomes active mid-session, right after a manual grant, and after a manual "Scan for models". Files without a streamed SHA-256 are skipped and logged (`LAI-workspace`); imported files appear as Installed.
